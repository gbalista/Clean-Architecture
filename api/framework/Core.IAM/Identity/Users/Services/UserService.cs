...
internal sealed partial class UserService(
    ...
    SignInManager<ArcUser> signInManager,
    ...
    ) : IUserService
{
    ...
    public async Task<string> ConfirmEmailAsync(string userId, string code, string tenant, CancellationToken cancellationToken)
    {
        ...
        _ = user ?? throw new Exceptions.ArcException("An error occurred while confirming E-Mail.");
        ...
        return result.Succeeded
            ...
            : throw new Exceptions.ArcException(string.Format("An error occurred while confirming {0}", user.Email));
    }
    ...
    {
        EnsureValidTenant();
        return await userManager.FindByEmailAsync(email.Normalize()) is ArcUser user && user.Id != exceptId;
    }
    ...
    {
        EnsureValidTenant();
        return await userManager.Users.FirstOrDefaultAsync(x => x.PhoneNumber == phoneNumber) is ArcUser user && user.Id != exceptId;
    }
    ...
    public async Task<RegisterUserResponse> RegisterAsync(RegisterUserCommand request, string origin, CancellationToken cancellationToken)
    {
        ...
        if (!result.Succeeded)
        {
            ...
            throw new Exceptions.ArcException("error while registering a new user", errors);
        }

        // add basic role
        await userManager.AddToRoleAsync(user, ArcRoles.Basic);
    ...
    public async Task ToggleStatusAsync(ToggleUserStatusCommand request, CancellationToken cancellationToken)
    {
        ...
        bool isAdmin = await userManager.IsInRoleAsync(user, ArcRoles.Admin);
        if (isAdmin)
        {
            throw new Exceptions.ArcException("Administrators Profile's Status cannot be toggled");
        }
    ...
    public async Task UpdateAsync(UpdateUserCommand request, string userId)
    {
        ...
        {
            user.ImageUrl = await storageService.UploadAsync<ArcUser>(request.Image, FileType.Image);
        ...
        if (!result.Succeeded)
        {
            throw new Exceptions.ArcException("Update profile failed");
        }
    }

    public async Task DeleteAsync(string userId)
    {
        ArcUser? user = await userManager.FindByIdAsync(userId);
        ...
        if (!result.Succeeded)
        {
            ...
            throw new Exceptions.ArcException("Delete profile failed", errors);
        }
    }

    private async Task<string> GetEmailVerificationUriAsync(ArcUser user, string origin)
    {
        EnsureValidTenant();
    ...
    public async Task<string> AssignRolesAsync(string userId, AssignUserRoleCommand request, CancellationToken cancellationToken)
    {
        ...
        if (await userManager.IsInRoleAsync(user, ArcRoles.Admin)
            && request.UserRoles.Exists(a => !a.Enabled && a.RoleName == ArcRoles.Admin))
        {
            // Get count of users in Admin Role
            int adminCount = (await userManager.GetUsersInRoleAsync(ArcRoles.Admin)).Count;
            ...
            if (user.Email == TenantConstants.Root.EmailAddress)
            {
                ...
                {
                    throw new Exceptions.ArcException("action not permitted");
                }
            }
            else if (adminCount <= 2)
            {
                throw new Exceptions.ArcException("tenant should have at least 2 admins.");
            }
        }
