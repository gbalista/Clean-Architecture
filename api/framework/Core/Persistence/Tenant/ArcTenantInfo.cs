...
public sealed class ArcTenantInfo : IArcTenantInfo
{
    public ArcTenantInfo()
    {
    }

    public ArcTenantInfo(string id, string name, string? connectionString, string adminEmail, string? issuer = null)
    {
        Id = id;
        Identifier = id;
        Name = name;
    ...
    public void SetValidity(in DateTime validTill) =>
        ValidUpto = ValidUpto < validTill
            ? validTill
            : throw new Exceptions.ArcException("Subscription cannot be backdated.");
    ...
    string? IArcTenantInfo.ConnectionString { get => ConnectionString; set => ConnectionString = value ?? throw new InvalidOperationException("ConnectionString can't be null."); }
}
